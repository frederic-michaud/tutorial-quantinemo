...
In this tutorial we will look at the basics of selection in quantiNemo. Selection can only act on phenotypic traits, this chapter will therefore also introduce quantitative traits. We will start with examples from single loci.    
...
As you just saw in the previous example, individuals are diploid in qN. Moreover, by default, alleles have a purely additive effect (though dominance can be implemented; see sub-section 8.2.2 of the manual), hence the pure addition of the two allelic effects in the previous example resulted in the genotype of the individual. And because by default environment does not influence the phenotype, the genotype and the phenotype will be the same in our current scenario.     

You may specifically set the effect of each allele yourself in a file similar to what we used in tutorial 2.2 and specified with `quanti_allelic_file` (see section 7.2 "defining loci" in the manual ). However, to write down the effect of 255 alleles requires quite some effort; and if your locus codes for some continuous trait (and nothing discrete such as 'eye colour'), you can instead let qN assign an effect to each allele. This is done by specifying a parameter of the normal distribution from which the allelic effect will be drawn, namely setting `quanti_allelic_var 4`.   
...
To observe and analyse the behaviour of our alleles, we will use a similar approach as in tutorial 2.1. However, because we now work with quantitative traits (as opposed to the neutral traits in the previous tutorial), we would also be interested in the mean genotypic value of the population (q.meanG_p) and of each individual (`quanti_save_geno_value`). instead of just saving the genotype of each individual (i.e all allele of every locus), we will also save the genotypic value of each individualquanti_environmental_model. Because our current simulation does not allow any influence of the environment on the phenotype, the genotypic and phenotypic value will be the same.   
...
```{r}
	...
	omegaA<-1
	stat_A<-read.table(myfileA<-"selection/i1/selection1_stats.txt",h=T)
	# b) low selection intenstiy (high omega)
	omegaB<-20
	stat_B<-read.table(myfileB<-"selection/i20/selection1_stats.txt",h=T)
...
If you would have run the simulation from the previous part of the tutorial long enough, only one or two alleles would have been left; selection would have selected the best allele to remain if drift had not earlier perchance already got rid of all diversity. However, our simulation did not include any sources of new genetic material. Mutation should set the system in a selection-mutation-drift equilibrium with non-zero diversity. We will demonstrate this precarious selection-mutation-drift balance in this short second tutorial on selection. Moreover, we will inspect how much of the equilibrium diversity is owing to selection, and how much can be assigned to drift solely.    

As a starting point of this simulation, we will use the same setup file as in the tutorial above and make some adaptations to suit our needs. For example, we introduce mutation in our model with a slightly high rate of 10-4.   
...
This will be our complete setup file. Safe your file (we called it `selection2.ini`) with these commands and run it.   
...
```{r fig.height = 4, fig.width = 12, fig.align = "center"}
	...
	# Select the file from the simulation with selection
	sel.mean<-read.table( spath<-"selection/mutation_with_selection/selection2_mean.txt",h=T); sel.var<-read.table(paste(substr(spath,1,nchar(spath)-9),"_var.txt",sep=""),h=T)
	
	# Select the file from the simulation without selection 
	drft.mean<-read.table( dpath<-"selection/mutation_without_selection/selection2_mean.txt",h=T); drft.var<-read.table(paste(substr(dpath,1,nchar(dpath)-9),"_var.txt",sep=""),h=T)
...
If you know the heritability of your trait, you may define the broad or narrow-sense heritability in qN instead the environmental variance directly (see section 8.3 Environmental effect of the manual). 
...
Open a new file in your text editor and save this under the name "fur_alleles.txt". I you already did the neutral marker tutorial, you should be able to write the FILE_INFO section yourself (see section 7.2 'Defining loci' of the manual); we need only three columns, one specifying the locus, one specifying the allele and one specifying the effect this allele should have.    
...
Now we need to add only one more layer of genetic interactions: the interactions between loci i.e. epistasis. Reading sub-section 8.2.3 'Epistatic effects' you will notice that the implementation in qN is very similar to that of dominance. So we will have to create another text file named `fur_epistasis.txt`
...
Now we have a setup file (we saved it as `fur1.ini`), four extra files defining our genetic architecture and a two mice ready to go. Make sure you ask the correct output before running the script. 
...
Below you can see our results(the related input file is called `fur2.ini` :
