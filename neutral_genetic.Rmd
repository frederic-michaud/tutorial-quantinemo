...
Instead we could have a look at section <tobechecked> 9.5 'Summary statistics' of the manual to see statistics potentially more interesting for our current simulation. Because we are currently interested in the accumulation of new alleles over time, we will pick three statistics commonly used to reflect allelic diversity: the number of alleles present, observed heterozygosity (HO) and expected heterozygosity (HS). 
...
The first line of this file contains four numbers, see the manual, <tobechecked> section 9.4 'Genotype output' for their meaning. Below are the names of all the loci, in our case only one, with the default name n(eutral trait)1_l(ocus)1.   
...
N.B. In this scenario we use this inifreq.txt file just to specify the initial frequencies, but you could also use it to specify, for example, a different mutation rate for each allele (see section <tobechecked> 9.1 'Architecture' of the manual for other possibilities).   
...
Since we want them to be saved in different and easly to recognise folder, we will add another sequential parameter containing the name of the folder in which we can save the result. We will also tell quantiNemo to associate the sequential parameter one by one, and not to run every possible scenario (see section <tobechecked> of the manual). Notice also that for readability reason, the first argument was extended over 5 lines (thanks to the \\ sign, c.f <tobechecked>), but this should really be understood as a single argument. 
...
Now let's check the linkage between the different alleles. As said before, qN cannot test this for you, you should calculate it yourself or use other software. Luckily the genotype output is by default in the FSTAT format (*.dat files; see the parameter `ntrl_genot_format`). This means we could use it immediately to import into pegas, a R package that allow, amoung other thing, to compute linkage disequilibrium 


We see the results of the test for genotypic disequilibrium. Because this test is based on randomization, the exact p-values may differ (though, if you used the same seed in qN, the r2 values should be the same!). However, all p-values should be higher than the threshold  p-value > 0.00333 (which is adjusted for multiple testing). These results confirm that at the first generation, no signs linkage disequilibrium can be found. **Question**:We told qN that three of our loci should be linked on the same chromosome; why do we not see this in generation 1? What parameter of the settings-file is responsible for this?
